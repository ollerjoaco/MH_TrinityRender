...
====================
*/
void CStudioModelRenderer::StudioSetupBonesPlayer(void)
{
	int i;
	double f;
	...
	if (m_pPlayerInfo && (m_pCurrentEntity->curstate.sequence < ANIM_FIRST_DEATH_SEQUENCE || m_pCurrentEntity->curstate.sequence > ANIM_LAST_DEATH_SEQUENCE) && (m_pCurrentEntity->curstate.sequence < ANIM_FIRST_EMOTION_SEQUENCE || m_pCurrentEntity->curstate.sequence > ANIM_LAST_EMOTION_SEQUENCE) && m_pCurrentEntity->curstate.sequence != ANIM_SWIM_1 && m_pCurrentEntity->curstate.sequence != ANIM_SWIM_2 && m_pCurrentEntity->curstate.sequence != ANIM_KICK_SEQUENCE)
	{
		int copy = 1;
...
void CStudioModelRenderer::StudioSetupBones(void)
{
	...
	for (i = 0; i < m_pStudioHeader->numbones; i++)
	{
		...
		if (pbones[i].parent == -1)
		{
			if (cl_righthand && cl_righthand->value > 0 && bIsViewModel && IEngineStudio.IsHardware() && g_iCurrentWeapon != WEAPON_KNIFE && !strstr(m_pCurrentEntity->model->name, "v_foot"))
			{
...
====================
*/
mstudioanim_t *CStudioModelRenderer::LookupAnimation(mstudioseqdesc_t *pseqdesc, int index)
{
...
int CStudioModelRenderer::StudioDrawModel(int flags)
{
	...
	if (m_pCurrentEntity->model && m_pCurrentEntity->model->name[7] == 'v' && m_pCurrentEntity->model->name[8] == '_')
	{
		if (strcmp(name, m_pCurrentEntity->model->name) != 0)
		{
			strcpy(name, m_pCurrentEntity->model->name);
			flTime = gEngfuncs.GetAbsoluteTime() + m_clTime - m_clOldTime;
		}

		if (gEngfuncs.GetAbsoluteTime() < flTime)
			return 0;
	}

	m_iFlippedVModel = IsFlippedViewModel();
...
void CStudioModelRenderer::CalculatePitchBlend(entity_state_t *pplayer)
{
	mstudioseqdesc_t *pseqdesc;
	int iBlend;
...
void CStudioModelRenderer::CalculateYawBlend(entity_state_t *pplayer)
{
	float dt;
	float flYaw;
...
int CStudioModelRenderer::StudioDrawPlayer(int flags, entity_state_t *pplayer)
{
	...
	//m_pRenderModel = m_pCurrentEntity->model;
	m_pRenderModel = IEngineStudio.SetupPlayerModel(m_nPlayerIndex);	//Fix single model
...
void CStudioModelRenderer::StudioSetupRenderer(int rendermode)
{
	...
	if (!m_bChromeShell)
	{
		...
		for (int i = 0; i < m_iNumModelLights; i++)
		{
			...
			if (0/*m_iFlippedVModel*/)	//check
			{
				flForward[0] = -flForward[0];
				flForward[1] = -flForward[1];
				flForward[2] = -flForward[2];
			}
			...
			{
				/*if (m_iFlippedVModel)
					VectorScale(flForward, -1, flForward);*/
...
void CStudioModelRenderer::StudioSetupLighting(void)
{
	...
	{
		/*if (m_iFlippedVModel)	//ilum mdls bugeado
			iret = StudioRecursiveLightPoint(pInfo, gBSPRenderer.m_pWorld->nodes, end, point, color);
		else*/
			iret = StudioRecursiveLightPoint(pInfo, gBSPRenderer.m_pWorld->nodes, point, end, color);		
	}
		

	if (!iret || gBSPRenderer.m_bFullBright)
	{
...
void CStudioModelRenderer::StudioDrawMesh(mstudiomesh_t *pmesh, mstudiotexture_t *ptex)
{
	...
	{
		glColor4f(0.25, 0.25, 0.25, m_fAlpha);
...
{
	if (m_pCurrentEntity->curstate.sequence >= m_pStudioHeader->numseq)	//Fix crash, smoke&flash grenades
	{
		m_pCurrentEntity->curstate.sequence = 0;
	}
	...
	if (m_pCvarModelsBBoxDebug->value > 0)
	{
		...
		for (int i = 0; i < 8; i++)
			glVertex3fv(Vector(m_pCurrentEntity->origin) + vBounds[i]);
...
void CStudioModelRenderer::Mod_LoadTexture(mstudiotexture_t *ptexture, byte *pbuffer, char *szmodelname)
{
	...
	//glGenTextures(1, &ptexture->index);
...
qboolean CStudioModelRenderer::IsFlippedViewModel(void)
{
	if (cl_righthand && cl_righthand->value > 0)
	{
		if (gEngfuncs.GetViewModel()->index == m_pCurrentEntity->index && g_iCurrentWeapon != WEAPON_KNIFE)
			return true;
	}
