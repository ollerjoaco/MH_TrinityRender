	...
	for (int i = 0; i < m_iNumDetailObjects; i++, pCurObject++)
	{
		...
		if (!m_bShaderSupport || m_pCvarWorldShaders->value < 1)
		{
			...
			for (int j = 0; j < MAX_DYNLIGHTS; j++)
			{
				...
				{
					if (m_pCurrentDynLight->frustum.CullBox((vec3_t)pCurObject->mins, (vec3_t)pCurObject->maxs))
						continue;
				}
				else
				{
...
void CBSPRenderer::EmitWaterPolys(msurface_t *fa)
{
		...
		|| m_pCurrentEntity->curstate.skin == CONTENTS_LAVA)
	{
		...
		for (glpoly_t *bp = fa->polys; bp; bp = bp->next)
		{
			...
			{
				if (bp->verts[i][2] != (m_pCurrentEntity->curstate.maxs[2] - 1))
					return;
			}
		}
	}
...
void CBSPRenderer::BuildLightmap(msurface_t *surf, int surfindex, color24 *out)
{
	...
	for (int i = 0; i < size; i++)
	{
		...
		{
			m_pBlockLights[i].r = m_pBlockLights[i].r*flIntensity*m_flLightScale;
			m_pBlockLights[i].g = m_pBlockLights[i].g*flIntensity*m_flLightScale;
			m_pBlockLights[i].b = m_pBlockLights[i].b*flIntensity*m_flLightScale;
		}

		ClampColor(&m_pBlockLights[i], &m_pBlockLights[i]);
	}
...
====================
*/
void CBSPRenderer::UpdateLightmaps(void)
{
	if (!m_pWorld)
		return;
...
void CBSPRenderer::ParseDetailTextureFile(void)
{
	...
	strcat(szLevelName, "_detailHD.txt");
...
void CBSPRenderer::CreateDecal(vec3_t endpos, vec3_t pnormal, const char *name, int persistent)
{
	...
	for (int i = 1; i < MAXRENDERENTS; i++)
	{
		...
		{
			mins = Vector(pEntity->origin) + Vector(pEntity->model->mins);
			maxs = Vector(pEntity->origin) + Vector(pEntity->model->maxs);
		}
