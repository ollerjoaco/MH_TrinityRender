...
void ClampColor(color24 *in, color24 *out)
{
	int r = in->r;
	int g = in->g;
	int b = in->b;
...
{
	// Copy sky name
	if (strcmp(gBSPRenderer.m_szSkyName, pparams->movevars->skyName))
	{
		strcpy(gBSPRenderer.m_szSkyName, pparams->movevars->skyName);
		gBSPRenderer.InitSky();
	}
...
void GenDetail(void)
{
	...
	strcat(szPathOut, "_detailHD.txt");
...
void SetupFlashlight(vec3_t origin, vec3_t angles, float time, float frametime)
{
	...
	if (gBSPRenderer.m_bShaderSupport && gBSPRenderer.m_pCvarWorldShaders->value >= 1)
	{
		...
		flashlight->frustum.SetFrustum(angles, flashlight->origin, flashlight->cone_size*1.2, 750);
